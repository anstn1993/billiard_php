...
<div class="mypage">
    ...
    <div class="mypage_child" id="mypage_article">
        ...
        <div id="post_category">
            <a href='mypost.php'>자유게시판</a>
            <?php
            if ($_SESSION['who'] == 'club_owner') {
                echo "<a href='my_event_post.php' style='background-color:#F2B08F'>당구장 이벤트</a>";
            } else if ($_SESSION['who'] == 'admin') {
                echo "
                  <a href='my_event_post.php' style='background-color:#F2B08F'>당구장 이벤트</a>
                  <a href='my_news_post.php'>당구 소식</a>
                  ";
            }
            ?>
        </div>
        <form action="delete_myevent.php" method="post" onsubmit="return delete_event()">
            ...
            <table id="mypost_event_table">
                ...
                if (!empty($_GET['page'])) {
                    if ($_GET['page'] <= 0 || $_GET['page'] > $total_page) {
                        ...
                        location.href='my_event_post.php';
                       </script>";
                    }
                }
                //전체 블럭 수
                ...
                for ($i = 0; $i < $total_data_num; $i++) {
                    ...
                    if (!empty($row[0])) {
                        ...
                        echo "
                          <tr>
                              <td>
                                <a href='event_detail.php?id={$row['number']}&page={$page}&mypost=yes'>{$row['number']}</a>
                              </td>
                              <td>
                                <a href='event_detail.php?id={$row['number']}&page={$page}&mypost=yes'>{$row['nickname']}</a>
                              </td>
                              <td>
                                <a href='event_detail.php?id={$row['number']}&page={$page}&mypost=yes'>{$row['title']}</a>
                              </td>
                              <td>
                                <a href='event_detail.php?id={$row['number']}&page={$page}&mypost=yes'>{$row['view_count']}</a>
                              </td>
                              <td>
                                <a href='event_detail.php?id={$row['number']}&page={$page}&mypost=yes'>{$row['comment_count']}</a>
                              </td>
                              <td>
                                <a href='event_detail.php?id={$row['number']}&page={$page}&mypost=yes'>{$date}</a>
                              </td>
                              <td>
                               <input type=\"checkbox\" name=\"myevent[]\" value=\"{$row['number']}\" class=\"myevent_check\">
                              </td>
                            </tr>
                          ";
                    }
        ...
        <div id="page">
            ...
            if ($total_data_num != 0) {
                ...
                echo ($page != 1) ? "<a href=\"my_event_post.php?page=1\">[처음으로]</a>" : "[처음으로]";

                echo ($current_block != 1) ? "<a href=\"my_event_post.php?page={$start_page_}\">[이전]</a>" : "[이전]";
                ...
                for ($i = $start_page; $i <= $end_page; $i++) {
                    if ($i == $page) {
                        echo "
                  <a href=\"my_event_post.php?page={$i}\" style=\"color:red; border:1px solid gray;\">{$i}</a>
                ";
                    } else {
                        echo "
                  <a href=\"my_event_post.php?page={$i}\">{$i}</a>
                ";
                    }
                ...
                echo ($current_block != $total_block) ? "<a href=\"my_event_post.php?page={$end_page_}\">[다음]</a>" : "[다음]";
                //현재 페이지가 마지막 페이지가 아닌 경우에만 마지막 페이지로 가는 버튼이 나오게 한다.
                echo ($page != $total_page) ? "<a href=\"my_event_post.php?page={$total_page}\">[마지막으로]</a>" : "[마지막으로]";
            }
...
<script type="text/javascript">
    ...
    function delete_event() {
        ...
        var arr_basket = document.getElementsByName("myevent[]");
