...
<html lang="en" dir="ltr">
            ...
            <?php
              if($_SESSION['who']=='club_owner'){
                echo "<a href='my_event_post.php' style='background-color:#F2B08F'>당구장 이벤트</a>";
                  ...
                  <a href='my_news_post.php' style='background-color:#F2B08F'>당구 소식</a>
                  ";
              }
            ?>
          </div>
          <form action="delete_mynews.php" method="post" onsubmit="return delete_news()">
            ...
            if(!empty($_GET['page'])){
              if($_GET['page']<=0 || $_GET['page']>$total_page){
                echo "<script>
                  ...
                  location.href='my_news_post.php';
                 </script>";
              }
            }
            //전체 블럭 수
              ...
              for($i=0; $i<$total_data_num; $i++){
                $row=mysqli_fetch_array($result);
                  ...
                  if(!empty($row[0])){
                    ...
                    <tr>
                      <td>
                        <a href='news_detail.php?id={$row['number']}&page={$page}&mypost=yes'>{$row['number']}</a>
                      </td>
                      <td class='list_title'>
                        <a href='news_detail.php?id={$row['number']}&page={$page}&mypost=yes'>{$row['title']}</a>
                      </td>
                      <td>
                        <a href='news_detail.php?id={$row['number']}&page={$page}&mypost=yes'>{$row['view_count']}</a>
                      </td>
                      <td>
                        <a href='news_detail.php?id={$row['number']}&page={$page}&mypost=yes'>{$row['comment_count']}</a>
                      </td>
                      <td>
                        <a href='news_detail.php?id={$row['number']}&page={$page}&mypost=yes'>{$date}</a>
                      </td>
                      <td>
                       <input type=\"checkbox\" name=\"mynews[]\" value=\"{$row['number']}\" class=\"myevent_check\">
                      </td>
                    </tr>
                    ";
                  }
        ...
        <div id="page">
          ...
          if($total_data_num!=0){
            //이전 블록으로 가기, 처음으로 가기 버튼
              ...
              echo ($page!=1)?"<a href=\"my_news_post.php?page=1\">[처음으로]</a>":"[처음으로]";

              echo ($current_block!=1)?"<a href=\"my_news_post.php?page={$start_page_}\">[이전]</a>":"[이전]";
            ...
            for($i=$start_page; $i<=$end_page; $i++){
              if($i==$page){
                echo "
                  <a href=\"my_news_post.php?page={$i}\" style=\"color:red; border:1px solid gray;\">{$i}</a>
                ";
              }else{
                echo "
                  <a href=\"my_news_post.php?page={$i}\">{$i}</a>
                ";
              }
            ...
            echo ($current_block!=$total_block)?"<a href=\"my_news_post.php?page={$end_page_}\">[다음]</a>":"[다음]";
              //현재 페이지가 마지막 페이지가 아닌 경우에만 마지막 페이지로 가는 버튼이 나오게 한다.
              echo ($page!=$total_page)?"<a href=\"my_news_post.php?page={$total_page}\">[마지막으로]</a>":"[마지막으로]";
  ...
  function delete_news(){
    ...
    var arr_basket=document.getElementsByName("mynews[]");
