...
//freeboard테이블을 내림차순 정렬했을 때 가장 첫번째 게시물의 번호를 담는 변수
$row_num = $row['number'];

//이 반복문은 전체 리스트의 사이즈를 구하기 위한 반복문이다.
for ($i = $row_num; $i > 0; $i--) {
    $sql = "
          SELECT*FROM freeboard
          WHERE number={$i}
...
<div class="mypage">
    ...
    <div class="mypage_child" id="mypage_article">
        ...
        <div id="post_category">
            <a href='mycomment.php' style="background-color:#F2B08F">자유게시판</a>
            <a href='mycomment_event.php'>당구장 이벤트</a>
            ...
            for ($i = $max_number; $i >= $min_number; $i--) {
                ...
                if ($exist_state != 0) {
                    ...
                    echo "
                       <tr>
                           <td>
                             <a href='freeboard_detail.php?id={$row['number']}&page={$page}&mycomment=yes'>{$row['number']}</a>
                           </td>
                           <td>
                             <a href='freeboard_detail.php?id={$row['number']}&page={$page}&mycomment=yes'>{$row['nickname']}</a>
                           </td>
                           <td class='list_title'>
                             <a href='freeboard_detail.php?id={$row['number']}&page={$page}&mycomment=yes'>{$row['title']}</a>
                           </td>
                           <td>
                             <a href='freeboard_detail.php?id={$row['number']}&page={$page}&mycomment=yes'>{$row['view_count']}</a>
                           </td>
                           <td>
                             <a href='freeboard_detail.php?id={$row['number']}&page={$page}&mycomment=yes'>{$row['comment_count']}</a>
                           </td>
                           <td>
                             <a href='freeboard_detail.php?id={$row['number']}&page={$page}&mycomment=yes'>{$date}</a>
                           </td>
                         </tr>
                       ";
                }
        ...
        <div id="page">
            ...
            if ($total_data_num != 0) {
                ...
                echo ($page != 1) ? "<a href=\"mycomment.php?page=1\">[처음으로]</a>" : "[처음으로]";

                echo ($current_block != 1) ? "<a href=\"mycomment.php?page={$start_page_}\">[이전]</a>" : "[이전]";
                ...
                for ($i = $start_page; $i <= $end_page; $i++) {
                    if ($i == $page) {
                        echo "
                    <a href=\"mycomment.php?page={$i}\" style=\"color:red; border:1px solid gray;\">{$i}</a>
                  ";
                    } else {
                        echo "
                    <a href=\"mycomment.php?page={$i}\">{$i}</a>
                  ";
                    }
                ...
                echo ($current_block != $total_block) ? "<a href=\"mycomment.php?page={$end_page_}\">[다음]</a>" : "[다음]";
                //현재 페이지가 마지막 페이지가 아닌 경우에만 마지막 페이지로 가는 버튼이 나오게 한다.
                echo ($page != $total_page) ? "<a href=\"mycomment.php?page={$total_page}\">[마지막으로]</a>" : "[마지막으로]";
