...
<div class="mypage">
    ...
    <div class="mypage_child" id="mypage_article">
        ...
        <div id="post_category">
            ...
            <a href='mycomment_news.php' style="background-color:#F2B08F">당구소식</a>
            ...
            for ($i = $max_number; $i >= $min_number; $i--) {
              ...
              ";
                ...
                if ($exist_state != 0) {
                  ...
                  <tr>
                    <td>
                      <a href='news_detail.php?id={$row['number']}'>{$row['number']}</a>
                    </td>
                    <td class='list_title'>
                      <a href='news_detail.php?id={$row['number']}'>{$row['title']}</a>
                    </td>
                    <td>
                      <a href='news_detail.php?id={$row['number']}'>{$row['view_count']}</a>
                    </td>
                    <td>
                      <a href='news_detail.php?id={$row['number']}'>{$row['comment_count']}</a>
                    </td>
                    <td>
                      <a href='news_detail.php?id={$row['number']}'>{$date}</a>
                    </td>
                  </tr>
                ";
                }
        ...
        <div id="page">
            ...
            if ($total_data_num != 0) {
                ...
                echo ($page != 1) ? "<a href=\"mycomment_news.php?page=1\">[처음으로]</a>" : "[처음으로]";

                echo ($current_block != 1) ? "<a href=\"mycomment_news.php?page={$start_page_}\">[이전]</a>" : "[이전]";
                ...
                for ($i = $start_page; $i <= $end_page; $i++) {
                    if ($i == $page) {
                        echo "
                  <a href=\"mycomment_news.php?page={$i}\" style=\"color:red; border:1px solid gray;\">{$i}</a>
                ";
                    } else {
                        echo "
                  <a href=\"mycomment_news.php?page={$i}\">{$i}</a>
                ";
                    }
                ...
                echo ($current_block != $total_block) ? "<a href=\"mycomment_news.php?page={$end_page_}\">[다음]</a>" : "[다음]";
                //현재 페이지가 마지막 페이지가 아닌 경우에만 마지막 페이지로 가는 버튼이 나오게 한다.
                echo ($page != $total_page) ? "<a href=\"mycomment_news.php?page={$total_page}\">[마지막으로]</a>" : "[마지막으로]";
            }
            ?>
        </div>
    </div>
