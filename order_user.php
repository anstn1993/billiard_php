...
for ($i = $row_num; $i > 0; $i--) {
    ...
    if ($_SESSION['account'] == $row['user_account']) {
    ...
    <div class="mypage_child" id="mypage_article">
                ...
                ";
                    ...
                    if ($_SESSION['account'] == $row['user_account']) {
                       ...
                       <tr>
                           <td style='border-bottom: 1px solid black;'  class='order_number'>
                             <a href='order_user_detail.php?order_number={$row['order_number']}'>{$row['order_number']}</a>
                           </td>
                           <td style='border-bottom: 1px solid black;'>
                             <a href='order_user_detail.php?order_number={$row['order_number']}'>
                             <img src='order_product_image_user/{$row['image']}' width='150px' height='100px'>
                             </a>
                           </td>
                           <td style='border-bottom: 1px solid black;'>
                             <a href='order_user_detail.php?order_number={$row['order_number']}'>{$row['name']}</a>
                           </td>
                           <td style='border-bottom: 1px solid black;'>
                             <a href='order_user_detail.php?order_number={$row['order_number']}'>";
                    ...
                    <tr>
                        <td class='order_number'>
                          <a href='order_user_detail.php?order_number={$row_compare['order_number']}'>
        ...
        <div id="page">
            ...
            if ($total_data_num != 0) {
                ...
                echo ($page != 1) ? "<a href=\"order_user.php?page=1\">[처음으로]</a>" : "[처음으로]";

                echo ($current_block != 1) ? "<a href=\"order_user.php?page={$start_page_}\">[이전]</a>" : "[이전]";
                ...
                for ($i = $start_page; $i <= $end_page; $i++) {
                    if ($i == $page) {
                        echo "
                  <a href=\"order_user.php?page={$i}\" style=\"color:red; border:1px solid gray;\">{$i}</a>
                ";
                    } else {
                        echo "
                  <a href=\"order_user.php?page={$i}\">{$i}</a>
                ";
                    }
                ...
                echo ($current_block != $total_block) ? "<a href=\"order_user.php?page={$end_page_}\">[다음]</a>" : "[다음]";
                //현재 페이지가 마지막 페이지가 아닌 경우에만 마지막 페이지로 가는 버튼이 나오게 한다.
                echo ($page != $total_page) ? "<a href=\"order_user.php?page={$total_page}\">[마지막으로]</a>" : "[마지막으로]";
            }
